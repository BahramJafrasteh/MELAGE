...
class GLAxisItem(GLGraphicsItem):
    ...
    def paint(self):
        ...
        x = np.arange(xmin, xmax + (xmax - xmin) / 10, (xmax - xmin) / 5).astype(np.int)
        y = np.arange(ymin, ymax + (ymax - ymin) / 10, (ymax - ymin) / 5).astype(np.int)
        z = np.arange(zmin, zmax + (zmax - zmin) / 10, (zmax - zmin) / 5).astype(np.int)
        xmax = x[-1];
        ymax = y[-1];
        zmax = z[-1]
